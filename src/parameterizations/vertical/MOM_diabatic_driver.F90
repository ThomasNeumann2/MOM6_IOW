...
#ifdef IOW
use MOM_wave_interface,      only : get_wave_method
#endif
...
subroutine diabatic(u, v, h, tv, BLD, fluxes, visc, ADp, CDp, dt, Time_end, &
  ...
  if(present(taux_bot) .and. present(tauy_bot)) then
    ...
    if (Waves%UseWaves .and. (get_wave_method(Waves) .eq. "IOW_WAVE")) then
!       tau_bot(i,j)   = sqrt(tau_bot(i,j)*tau_bot(i,j) + Waves%tauw(i,j)*Waves%tauw(i,j))
       tau_bot(i,j)   = sqrt(tau_bot(i,j)*tau_bot(i,j) + Waves%tauw(i,j)*Waves%tauw(i,j))
       Waves%bot_stress(i,j) = tau_bot(i,j)
    endif
    enddo; enddo
  endif
#endif
